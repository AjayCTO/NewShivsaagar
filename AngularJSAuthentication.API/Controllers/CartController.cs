        ...
        public List<CartListViewModel> GetCart(string UserName)
        {
            try
            {
                ...
                foreach (var _item in _cartlistData)
                {
                    ...
                    _newlistwm.Add(new CartListViewModel() { Image = _imagePath, Id = _item.Id, ProductId = _item.ProductId, CustomerId = _item.CustomerId, ProductName = _item.Product.ProductName, ProductDescription = _item.Product.Description, ProductPrice = _productQuantity.Count() > 0 ? _productQuantity.FirstOrDefault().UnitPrice : 0 });
                }
